...
OUT_PNG = PANELS_DIR / "Figure4C_sigma_eff_by_phase.png"
OUT_PDF = PANELS_DIR / "Figure4C_sigma_eff_by_phase.pdf"
OUT_TSV = DERIVED_DIR / "figure4C_sigma_eff_stats.tsv"
...
def main() -> None:
    ...
    df["sigma_eff"] = pd.to_numeric(df["sigma_eff"], errors="coerce")
    ...
    x_dx = pooled.loc[
        pooled["clinical_timepoint_coarse"] == "DX",
        "sigma_eff"
    ].dropna().to_numpy()

    x_rel = pooled.loc[
        pooled["clinical_timepoint_coarse"] == "REL",
        "sigma_eff"
    ].dropna().to_numpy()

    if len(x_dx) == 0 or len(x_rel) == 0:
        raise ValueError("Need both DX and REL main-analysis samples for Panel 4C.")

    # EOI/REM points shown individually, not pooled
    ...
    if not eoi.empty:
        eoi_sorted = eoi.sort_values("sigma_eff").copy()
    ...
    ax.set_ylabel(r"Effective instability / diffusion proxy ($\sigma_{\mathrm{eff}}$)")

    style_axis(
        ax,
        "C",
        "Effective instability by phase",
        title_x=0.12
    )
    ...
    if len(eoi):
        y_candidates.append(np.nanmax(eoi["sigma_eff"]))
    ...
    if len(eoi):
        y_min_candidates.append(np.nanmin(eoi["sigma_eff"]))
