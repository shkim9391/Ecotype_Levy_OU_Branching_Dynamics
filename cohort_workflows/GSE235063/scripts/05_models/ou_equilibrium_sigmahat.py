...
ROOT = Path("/Ecotype_OU_Branching/GSE235063/derived_dx_primary_training")
df = pd.read_csv(ROOT / "dx_ou_ilr_branch_ready.csv")

predictors = ["PC1", "PC2", "is_blood"]
responses = [
...
def fit_suite(df_sub: pd.DataFrame, tag: str):
    ...
    perf_df.to_csv(ROOT / f"ou_equilibrium_performance__{tag}.csv", index=False)
    coef_df.to_csv(ROOT / f"ou_equilibrium_coefficients__{tag}.csv", index=False)
    pred_df.to_csv(ROOT / f"ou_equilibrium_predictions__{tag}.csv", index=False)
    sigma_hat.to_csv(ROOT / f"ou_equilibrium_sigmahat__{tag}.csv")
...
print("\n=== FILES WRITTEN ===")
for p in sorted(ROOT.glob("ou_equilibrium_*")):
    print(p)
