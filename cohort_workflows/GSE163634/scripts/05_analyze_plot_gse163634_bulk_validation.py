...
def plot_trajectories(ax, deltas: pd.DataFrame, axes: List[str]) -> None:
    ...
    # use up to 3 axes for readability
    chosen = chosen[:3]
    x_offsets = np.arange(len(chosen)) * 3.0
    for i, axis in enumerate(chosen):
        ...
        for _, row in sub.iterrows():
            ax.plot([x0, x1], [row[fcol], row[tcol]], linewidth=0.8, alpha=0.7)
            ax.scatter([x0, x1], [row[fcol], row[tcol]], s=12)
        med_from = sub[fcol].median()
        med_to = sub[tcol].median()
        ax.plot([x0, x1], [med_from, med_to], linewidth=2.5)
    xticks = []
    xtlabs = []
    for i, axis in enumerate(chosen):
        x0 = x_offsets[i]
        xticks.extend([x0, x0 + 1.0])
        xtlabs.extend([f"{axis}\nDx", f"{axis}\nR1"])
...
def main() -> None:
    ...
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.0], width_ratios=[1.0, 1.4])
    axA = fig.add_subplot(gs[:, 0])
    axB = fig.add_subplot(gs[0, 1])
    ...
    fig_png = outdir / "Figure_GSE163634_bulk_validation_main_5axis.png"
    fig_pdf = outdir / "Figure_GSE163634_bulk_validation_main_5axis.pdf"
    ...
    if not lvsc.empty and not scores.loc[scores["is_leukemia"] == False].empty:
        ...
        sup1_png = outdir / "Figure_SX_GSE163634_leukemia_vs_control_5axis.png"
        sup1_pdf = outdir / "Figure_SX_GSE163634_leukemia_vs_control_5axis.pdf"
    ...
    if not r2_stats.empty:
        ...
        sup2_png = outdir / "Figure_SX_GSE163634_r1_to_r2_5axis.png"
        sup2_pdf = outdir / "Figure_SX_GSE163634_r1_to_r2_5axis.pdf"
